// /api/scrape.js
// ==========================================================
// 🏡 Scraper Immo Pro – SeLoger (RapidAPI) + Etalab DVF
// ==========================================================

const cache = new Map();
const RAPID_KEY = process.env.RAPIDAPI_KEY;

// ==================== 1️⃣ Codes Départements ====================
...
// ==================== 3️⃣ Prix médian marché (Etalab DVF) ====================
...
// ==================== 4️⃣ Scraper SeLoger (RapidAPI) ====================
async function fetchSeLoger(depName) {
  try {
    const query = encodeURIComponent(`appartement maison à vendre ${depName}`);
    const url = `https://seloger.p.rapidapi.com/properties/list?zip=${query}&pageIndex=1&pageSize=25`;
    ...
    console.warn("⚠️ Erreur SeLoger:", err.message);
    return [];
  }
}
...
export default async function handler(req, res) {
  try {
    ...
    const cacheKey = `seloger_${codeDep}`;
    ...
    const [median, annonces] = await Promise.all([
      getMedian(codeDep),
      fetchSeLoger(departement)
    ]);

    // === Fallback Etalab ===
    if (!annonces.length) {
      console.warn("⚠️ Fallback sur Etalab DVF uniquement.");
      const dummy = [{
      ...
      return res.status(200).json({ medianRef: median, annonces: dummy });
    }
