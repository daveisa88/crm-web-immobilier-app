// ==========================================================
// 🏡 Scraper Immo Pro – Apify (SeLoger) + Etalab DVF
// Compatible CommonJS (Electron) + Vercel (ESM via import dynamique)
// ==========================================================

// === Import dynamique compatible Vercel ===
const fetch = (...args) =>
...
// - APIFY_TOKEN
// - APIFY_SELOGER_ACTOR_ID
// - DEFAULT_PROVIDER (par ex: "seloger")
const APIFY_TOKEN = process.env.APIFY_TOKEN;
const APIFY_SELOGER_ACTOR_ID =
    process.env.APIFY_SELOGER_ACTOR_ID || "lexis-solutions/seloger-scraper";
const DEFAULT_PROVIDER = process.env.DEFAULT_PROVIDER || "seloger";

// === Codes départements France ===
...
// === Prix médian du marché (DVF Etalab) ===
async function getMedian(codeDep) {
    try {
        const url = `https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets/demandes-de-valeurs-foncieres/records?where=code_departement="${codeDep}" and valeur_fonciere>20000 and surface_reelle_bati>10&limit=400`;
...
// === Scraper Apify (SeLoger Actor) ===
async function fetchSeLoger(depName) {
    if (!APIFY_TOKEN) throw new Error("APIFY_TOKEN manquant.");
    try {
        const url = `https://api.apify.com/v2/acts/${APIFY_SELOGER_ACTOR_ID}/runs?token=${APIFY_TOKEN}`;
        const body = {
            location: depName,
            maxItems: 25,
            dealType: "vente"
        ...
        if (!datasetId) throw new Error("Aucun dataset trouvé.");

        const dataUrl = `https://api.apify.com/v2/datasets/${datasetId}/items?token=${APIFY_TOKEN}`;
        const dataRes = await fetch(dataUrl);
        const annonces = await dataRes.json();

        return annonces
            .filter(a => a.price && a.surface)
            .map(a => ({
                titre: a.title || "Annonce SeLoger",
                departement: depName,
                prix: a.price,
                surface: a.surface,
                ...
                ville: a.city || "",
                lien: a.url || "",
                source: "SeLoger (Apify)"
            }));
    } catch (err) {
        console.warn("⚠️ Erreur Apify SeLoger:", err.message);
        return [];
    }
}
...
export default async function handler(req, res) {
    try {
        ...
        if (cache.has(cacheKey) && Date.now() - cache.get(cacheKey).time < 3600 * 1000) {
            return res.status(200).json(cache.get(cacheKey).data);
        }

        const [median, annonces] = await Promise.all([
            getMedian(codeDep),
            provider === "seloger" ? fetchSeLoger(departement) : []
        ]);

        if (!annonces.length) {
            const fallback = [{
                titre: `Marché moyen ${departement}`,
                departement,
                prix: median * 80,
                surface: 80,
                prixM2: median,
            ...
            return res.status(200).json({ medianRef: median, annonces: fallback });
        }
        ...
        const payload = {
            medianRef: median,
            annonces: enrichies.sort((a, b) => b.viabilite - a.viabilite).slice(0, 50)
        };
